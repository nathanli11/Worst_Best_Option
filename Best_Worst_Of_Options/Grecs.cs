...
public static class Grecs
{
    ...
    /// </summary>
    public static double[] Delta(Option option, Market market, double riskFreeRate,
        double h = 0.01, int numPaths = 5000, int days=252)
    {
        ...
        for (int i=0; i < n; i++)
        {
            ...
            double VPlus = option.PriceOption(market, riskFreeRate, numPaths, days); // On suppose que la fonction price est celle la

            // Bump down
            stock.HistoricalPrices[lastDate] = original * (1 - h);
            double VMinus = option.PriceOption(market, riskFreeRate, numPaths, days);
